 ...
 * say "5 heads" on a 3-head product. Only two things still fall back to
 ...
 * "Blister with oil 3 heads"-style option string — Shopify has no
  ...
  for (const id of knownIds) {
    ...
    const variants: SyncedVariant[] = priced.map((v) => {
      ...
      return {
        ...
        // its raw option string ("Blister with oil 3 heads") would
