 ...
 *
 * Claim policy: hardware properties (pulse, intensity levels, head types,
 * materials, weight, dimensions) are stated as fact; acupressure and meridian
 * point maps are framed as a tradition drawn from classical Chinese medicine,
 * never as a proven mechanism or a health outcome. The pen is a consumer
 * wellness device, not a medical one, and is not cleared by the FDA or Health
 * Canada — no listing copy may imply otherwise. See content/copy.ts for the
 * full claim policy this file's data has to stay inside.
 */
