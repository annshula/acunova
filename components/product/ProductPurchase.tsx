...
function ProofLink({
  ...
  return (
    ...
    >
      {icon}
      {children}
      <ChevronRightIcon className="h-3 w-3 shrink-0 -translate-x-1 text-ink-mute opacity-0 transition-[transform,opacity] duration-300 ease-(--ease-out-expo) group-hover:translate-x-0 group-hover:opacity-100" />
    </a>
  );
}
        ...
        <div>
          ...
          <p className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-[0.8rem]">
            <ProofLink
              ...
              icon={
                <WrenchIcon className="h-3.5 w-3.5 shrink-0 text-gold transition-colors duration-200 group-hover:text-ink" />
              }
            >
              Hand-built
            </ProofLink>
          ...
          <div className="mt-6">
            ...
            {descCanExpand && (
              <button
                ...
                className="group mt-2 inline-flex items-center gap-1.5 text-[0.76rem] font-semibold tracking-[0.08em] text-ink uppercase transition-colors duration-200 hover:text-gold"
              >
