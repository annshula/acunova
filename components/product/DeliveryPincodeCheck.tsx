  ...
  return (
    <div className="rounded-xl border border-line bg-linen px-4 py-3.5">
      <form onSubmit={onSubmit} className="flex items-center gap-2.5">
        <span
          aria-hidden
          className="grid size-9 shrink-0 place-items-center rounded-full bg-accent-soft text-gold"
        >
        ...
        <button
          ...
          className="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-full bg-linear-to-b from-gold-hot to-gold px-3.5 font-display text-[0.68rem] font-semibold tracking-widest text-on-accent uppercase transition-[transform,opacity] duration-300 hover:-translate-y-0.5 disabled:opacity-50 disabled:hover:translate-y-0"
        >
          {state.kind === "loading" && (
        ...
        {state.kind === "result" && (
          ...
          >
            ...
            >
              {state.result.ok ? (
                <>
                  <span className="mt-0.5 grid size-4 shrink-0 place-items-center rounded-full bg-gold text-on-accent">
