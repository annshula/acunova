...
/**
 * Risk-reversal close for the pen page. The headline promises a strong
 ...
 * content/product-pen.ts (covered).
 */
export default function PenClose({
  content,
}: {
  /** Close copy — pass content/pitches.ts closeGeneric for non-flagship products, default is the flagship pen copy. */
  content?: CloseContent;
}) {
  const c = content ?? pen.covered;
  ...
  const chips = [
    ...
    {
      icon: <DeviceIcon className="h-4.5 w-4.5" />,
      label: "The complete kit",
      body: "Pen, all four heads, case and quick-start card, counted at the bench before it ships.",
    },
  ];
  ...
  return (
    ...
    >
      ...
      <Reveal className="mt-12 flex flex-col items-center text-center lg:mt-14">
        <a
          href="#buy"
          className="group relative flex h-14 items-center gap-2.5 overflow-hidden rounded-full bg-linear-to-b from-gold-hot to-gold px-10 font-display text-[0.88rem] font-semibold tracking-widest whitespace-nowrap text-on-accent uppercase transition-transform duration-400 ease-(--ease-out-expo) hover:-translate-y-0.5 active:scale-[0.98]"
        >
          <span
            aria-hidden
