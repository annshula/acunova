            ...
            {!isLast && (
              <span className="absolute top-6 bottom-0 left-[0.6875rem] w-0.5 overflow-hidden rounded-full bg-line">
                <motion.span
                  ...
                  className={cn(
                    "block h-full w-full",
                    step.state === "pending" ? "bg-line" : "bg-gold",
                  )}
                />
              </span>
            )}
...
function TimelineDot({ state }: { state: TrackingStep["state"] }) {
  if (state === "current") {
    return (
      <span className="relative grid size-6 place-items-center rounded-full bg-gold ring-4 ring-accent-soft">
        <motion.span
          aria-hidden
  ...
  if (state === "done") {
    return (
      <span className="grid size-6 place-items-center rounded-full bg-gold text-on-accent">
